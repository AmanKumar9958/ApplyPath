...
export default function LiquidEther({
  ...
  /** Maximum frames per second (approx). Set to 30 or 45 for lower CPU/GPU usage. Null/undefined = uncapped */
  maxFPS = 45,
  /** Disable effect on narrow/mobile screens */
  disableOnMobile = true,
  /** Minimum width to enable effect when disableOnMobile is true */
  minWidth = 768,
  /** Dynamically lower sim resolution when tab not focused or when fps budget tight */
  dynamicResolution = true,
  /** Lower bound multiplier for dynamic resolution */
  minDynamicResolution = 0.25
}) {
...
`;
    ...
    class WebGLManager {
      constructor(props) {
        ...
        this.frameInterval = props.maxFPS ? 1000 / props.maxFPS : 0;
      ...
      render(now) {
        ...
        if (this.frameInterval) {
          ...
          if (delta < this.frameInterval) return; // skip frame
          this.lastFrameTime = now;
        }
        if (this.autoDriver) this.autoDriver.update();
        Mouse.update();
        Common.update();
        // Dynamic resolution: if delta between frames large -> temporarily lower resolution
        if (dynamicResolution && this.frameInterval) {
          const sim = this.output?.simulation;
          if (sim) {
            const targetRes = resolution;
            // If frames are being skipped a lot (delta > 1.75 * frameInterval) lower resolution
            const recentDelta = now - this.lastFrameTime;
            const shouldLower = recentDelta > this.frameInterval * 1.75;
            if (shouldLower && !this.lowResActive) {
              sim.options.resolution = Math.max(minDynamicResolution, targetRes * 0.6);
              sim.resize();
              this.lowResActive = true;
            } else if (!shouldLower && this.lowResActive) {
              sim.options.resolution = targetRes;
              sim.resize();
              this.lowResActive = false;
            }
          }
    ...
    const isSmall = disableOnMobile && window.innerWidth < minWidth;
    if (isSmall) {
      // Do not initialize heavy WebGL; keep container empty
      return;
    }
